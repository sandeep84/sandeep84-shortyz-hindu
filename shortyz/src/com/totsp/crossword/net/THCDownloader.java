...
public class THCDownloader extends AbstractDownloader {
    ...
    private boolean downloadAndParseGrid(Date date) {
        ...
        try {
            ...
            Pattern p = Pattern.compile("<span\\s*class=\"author\">(.*?)</span>");
            Matcher m = p.matcher(html_string);
            while (m.find()) {
            	this.author = m.group(1);
            }
            
            p = Pattern.compile("<h1\\s*class=\"detail-title\">(.*?)</h1>");
        	...
        	//Now, the clues
            numClues = 0;
            p = Pattern.compile("<p\\s*class=\"body\">(.*?)</p>", Pattern.MULTILINE | Pattern.DOTALL);
            m = p.matcher(html_string);
            while (m.find()) {
            	String clue = m.group(1);
            	clue = android.text.Html.fromHtml(clue).toString();
